...
%[gamma, gdev, gstatsg] = glmfit(wfe, Dta.PTA,'binomial','link','probit','constant','off');   
%segamma = gstatsg.se;
%V = gstatsg.covb;

[gamma, cov_Hessian, ~, ~, ~] = PROBIT( Dta.PTA , wfe, 3);
...
b2 = fsolve(@foc_psn_new, bin, options, Dta.X, zfe, wfe, Dta.PTA, gamma);
D2 = varmat_new(b2, Dta.X, zfe, wfe, Dta.PTA, gamma, V);
se2 = sqrt(diag(D2));
